 ...
 */
package org.shopping.cockpits.setup;
...
import org.shopping.cockpits.constants.ShoppingCockpitsConstants;
 ...
 */
@SystemSetup(extension = ShoppingCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(ShoppingCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(ShoppingCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						ShoppingCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/shoppingcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
